...
class _DailyReportScreenState extends State<DailyReportScreen> {
    ...
    try {
      ...
      switch (_selectedPeriod) {
        ...
        case ReportPeriod.month:
          ...
          _periodDates = List.generate(
              daysInMonth, (i) => startDate.add(Duration(days: i)));
          break;
      }
      ...
      if (_chartSelectedDate != null) {
        ...
        filterStart = DateTime(_chartSelectedDate!.year,
            _chartSelectedDate!.month, _chartSelectedDate!.day);
      ...
      _transactions = allTxns.where((t) {
        return t.transactionDate
                .isAfter(filterStart.subtract(const Duration(seconds: 1))) &&
        ...
        children: [
          IconButton(
            ...
            onPressed: () {
              setState(() {
                if (_selectedDateRange != null) {
                  final duration = _selectedDateRange!.end
              ...
              setState(() {
                if (_selectedDateRange != null) {
                  ...
                  if (nextStart
                    ...
                    _selectedDateRange = DateTimeRange(
                      start: nextStart,
                      end: nextEnd.isAfter(DateTime.now())
                          ? DateTime.now()
                          : nextEnd,
                    );
                    _selectedDate = _selectedDateRange!.start;
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      await Printing.sharePdf(
          bytes: pdfBytes,
          filename: 'Report_${DateFormat('yyyy-MM-dd').format(start)}.pdf');
    } catch (e) {
  ...
  Future<void> _exportExcel() async {
    ...
    try {
      ...
      for (var t in _transactions) {
        buffer.writeln(
            '${t.isBuyTransaction ? "Buy" : "Sell"},"${t.customerName}",${DateFormat('yyyy-MM-dd HH:mm').format(t.transactionDate)},${t.totalAmount}');
      }
      ...
      for (var e in _expenses) {
        buffer.writeln(
            '"${e.category.name}","${e.notes?.replaceAll('"', '""') ?? ""}",${DateFormat('yyyy-MM-dd HH:mm').format(e.date)},${e.amount}');
      }

      final bytes = utf8.encode(buffer.toString());
      await Printing.sharePdf(
          bytes: bytes,
          filename: 'Report_${DateFormat('yyyy-MM-dd').format(start)}.csv');
    } catch (e) {
      ScaffoldMessenger.of(context)
  ...
  Future<void> _printReport() async {
    ...
    try {
      ...
      await Printing.layoutPdf(
          ...
          name: 'Report_${DateFormat('yyyy-MM-dd').format(start)}');
    } catch (e) {
      ScaffoldMessenger.of(context)
